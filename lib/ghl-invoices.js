...
// Finds the Stripe Customer ID behind a paid invoice's transaction — needed
// to auto-charge the same customer again later for the final balance (see
// routes/bookings.js's ensureStripeCustomerId). Only ever called after
// confirming the invoice is actually paid; returns null if no matching
// transaction or Stripe customer reference is found (e.g. paid a different
// way, or GHL hasn't recorded it yet).
export async function getStripeCustomerIdForInvoice(invoiceId, locationId, token) {
  ...
  const tx = transactions.find((t) => t.entityId === invoiceId);
  return tx?.chargeSnapshot?.charges?.data?.[0]?.customer ?? null;
}
