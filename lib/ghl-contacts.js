...
export async function findDogsForContact({ locationId, contactId, dogFieldMap, token }) {
  ...
  // Compare as strings — GHL doesn't document the two endpoints as
  // guaranteeing matching id types, and a mismatch here would silently look
  // identical to "no dogs found" again.
  const relations = (result.data.relations ?? []).filter((r) => String(r.associationId) === String(association.id));
