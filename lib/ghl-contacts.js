...
// Updates only the given vaccine-expiration fields on one specific dog record,
// by its exact record ID. Used instead of relying on GHL's own "Update
// Associated Record for Contact" workflow action, which has no way to target
// a single record — it updates every record under an association label, so a
// multi-dog household renewing one dog's vaccines would silently overwrite
// every other dog's data too (confirmed live against a real sandbox record).
export async function updateDogVaccineFields({ dogObjectId, objectKey, locationId, vaccineDates, token }) {
  const result = await ghlRequest('PUT', `/objects/${objectKey}/records/${dogObjectId}`, {
    params: { locationId },
    body: { properties: vaccineDates },
    token,
  });
  if (!result.ok) {
    throw new Error(`GHL dog vaccine update failed: ${result.status} ${JSON.stringify(result.data)}`);
  }
